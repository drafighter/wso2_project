...
public class RestApiService {
	...
	private <T> ResponseEntity<T> callApiEndpoint(final String url, HttpMethod httpMethod, HttpHeaders httpHeaders, Object body, Class<T> clazz, Map<String, String> params) throws URISyntaxException {
		...
		if (httpHeaders.getContentType() != null && (httpHeaders.getContentType().equals(MediaType.APPLICATION_FORM_URLENCODED))) {
		...
		try {
			return restTemplateBuilder.build().exchange(restUrl, httpMethod, new HttpEntity<>(body, httpHeaders), clazz);
	...
	@SuppressWarnings("unchecked")
	public <T> ResponseEntity<T> postJoinOn(String url, HttpHeaders httpHeaders, Object body) {
		return post(url, httpHeaders, body, (Class<T>) Object.class, null);
	}

	public <T> ResponseEntity<T> postJoinOn(String url, HttpHeaders httpHeaders, Object body, Class<T> clazz) {
		return postJoinOn(url, httpHeaders, body, clazz, null);
	}

	public <T> ResponseEntity<T> postJoinOn(String url, HttpHeaders httpHeaders, Object body, Class<T> clazz, Map<String, String> params) {
		try {
			return callApiEndpointpostJoinOn(url, HttpMethod.POST, httpHeaders, body, clazz, params);
		} catch (URISyntaxException e) {
			log.error(e.getMessage());
		}
		return null;
	...
	private <T> ResponseEntity<T> callApiEndpointpostJoinOn(final String url, HttpMethod httpMethod, HttpHeaders httpHeaders, Object body, Class<T> clazz, Map<String, String> params) throws URISyntaxException {
		String restUrl = url;
