 ...
 * </pre>
 */
package com.amorepacific.oneap.auth.join.vo;
...
import com.amorepacific.oneap.auth.api.vo.ivo.CreateCustVO.CicuedCuAddrTcVo;
import com.amorepacific.oneap.auth.api.vo.ivo.CreateCustVO.CicuedCuChCsTcVo;
import com.amorepacific.oneap.auth.api.vo.ivo.CreateCustVO.CicuedCuTncaTcVo;
import com.amorepacific.oneap.auth.api.vo.ivo.CreateCustVO.CicuemCuAdtInfTcVo;
import com.amorepacific.oneap.auth.api.vo.ivo.CreateCustVO.CicuemCuOptiCsTcVo;
...
public class JoinData {
	...
	public static CreateCustVO buildIntegratedOnlineCreateCustomerData(JoinRequest joinRequest, Channel channel) {
		...
		if (joinMarketings != null && !joinMarketings.isEmpty()) {
			for (Marketing joinMarketing : joinMarketings) {
				...
				//20230405  선택약관 추가로 분기처리
				if(marketing.getChCd().equals("030")) {
					marketing.setChCd(joinRequest.getChcd());
					
					for(Marketing vo : joinRequest.getMarketings()) {
						if(joinRequest.getChcd().equals(vo.getChCd())) {
							marketing.setSmsOptiYn(vo.getSmsAgree());
						}
					}
				}
				boolean marketingbool = true;
				if(createCustVo.getCicuemCuOptiCsTcVo() != null) {
					for(CicuemCuOptiCsTcVo vo : createCustVo.getCicuemCuOptiCsTcVo()){
						if(marketing.getChCd().equals(vo.getChCd())) {
							marketingbool = false;
						}
					}
				}
				...
				//20230405
				if(chcsTcVo.getChCd().equals("030")) {
					chcsTcVo.setChCd(joinRequest.getChcd());
				}
				//20230405 선택약관 추가로 분기처리
				boolean chcsTcVobool = true;
				if(createCustVo.getCicuedCuChCsTcVo() != null) {
					for(CicuedCuChCsTcVo vo : createCustVo.getCicuedCuChCsTcVo()){
						if(chcsTcVo.getChCd().equals(vo.getChCd())) {
							chcsTcVobool = false;
						}
					}
				}
				...
				if(config.isMarketingSyncBpEnable(channel.getChCd(), profile)) {
					...
					if (joinMarketings != null && !joinMarketings.isEmpty()) {
						for (Marketing joinMarketing : joinMarketings) {
							...
							//20230405 선택약관 추가로 분기처리
							if ("030".equals(joinMarketing.getChCd())) {
								marketing.setSmsOptiYn(joinMarketing.getSmsAgree()); // SMS수신동의여부
							}
						}
					}
			...
			if (StringUtils.hasText(joinRequest.getLoginid()) /* && !"stg".equals(profile) */) { // 온라인 ID 입력여부 체크한 경우에만 030을 포함한 6개 채널 동시 가입 처리(stg환경 제외:뷰티포인트 없음)
				for(Object chCd : config.getBpChannelCodes()) {
					if(!chCd.equals(joinRequest.getChcd())) { // 2023-01-06 동시 가입되는 6개 채널 중 joinChCd는 제외 (고객통합 오류방지)
						CicuemCuOptiCsTcVo marketing = new CicuemCuOptiCsTcVo();
	...
	public static CreateCustVO buildIntegratedOfflineCreateCustomerData(JoinRequest joinRequest, Channel channel) {
		...
		if (joinMarketings != null && !joinMarketings.isEmpty()) {
			for (Marketing joinMarketing : joinMarketings) {
				...
				// 제휴사 멤버십 연동 시 추가 파라미터
				if (StringUtils.hasText(joinRequest.getPrcnLnkgStorNo()) && joinRequest.getChcd().equals(joinMarketing.getChCd())) {
					chcsTcVo.setPrcnLnkgStorNo(joinRequest.getPrcnLnkgStorNo());
				} else if (StringUtils.hasText(joinRequest.getPrcnLnkgStorNo()) && OmniConstants.NAVER_STORE_CHCD.equals(joinMarketing.getChCd())) { // 네이버 스마트 스토어(401) 인 경우 강제로 401 채널의 interlockSellerNo 주입 
					chcsTcVo.setPrcnLnkgStorNo(config.getChannelApi(OmniConstants.NAVER_STORE_CHCD, "sellerno", profile));
				}
			...
			// 030을 포함한 6개 채널 동시 가입 처리 2021-08-04 hjw0228
			if (StringUtils.hasText(joinRequest.getLoginid()) && "N".equals(channel.getOflnChnYn())) { // 온라인 ID 입력여부 체크한 경우에만 030을 포함한 6개 채널 동시 가입 처리(stg환경 제외:뷰티포인트 없음)
				for(Object chCd : config.getBpChannelCodes()) {
					CicuemCuOptiCsTcVo marketing = new CicuemCuOptiCsTcVo();
			...
			if(OmniConstants.OSULLOC_OFFLINE_CHCD.equals(joinChCd) && (OmniConstants.OSULLOC_OFFLINE_CHCD.equals(channel.getChCd()) || OmniConstants.OSULLOC_CHCD.equals(channel.getChCd()))) {
				...
				// Mandatory
				marketing.setChCd(OmniConstants.OSULLOC_DEPARTMENT_CHCD);
				// Optional
				// join-on 가입하는 API에 수신동의 항목 추가
				if (joinMarketings != null && !joinMarketings.isEmpty()) {
					for (Marketing joinMarketing : joinMarketings) {
				...
				// Mandatory
				chcsTcVo.setChCd(OmniConstants.OSULLOC_DEPARTMENT_CHCD);
				chcsTcVo.setFscrId("OCP");
				chcsTcVo.setLschId("OCP");
				// Optional & Mandatory
				// 고객가입시 사용한 매체 관리를 위한 코드(W:WEB, M:MOBILE, A:APP)가 Web 일 경우 chcsNo 가 웹 아이디가 됨.
			...
			if(OmniConstants.OSULLOC_DEPARTMENT_CHCD.equals(joinChCd) && (OmniConstants.OSULLOC_DEPARTMENT_CHCD.equals(channel.getChCd()) || OmniConstants.OSULLOC_CHCD.equals(channel.getChCd()))) {
				...
				// Mandatory
				marketing.setChCd(OmniConstants.OSULLOC_OFFLINE_CHCD);
				// Optional
				// join-on 가입하는 API에 수신동의 항목 추가
				if (joinMarketings != null && !joinMarketings.isEmpty()) {
					for (Marketing joinMarketing : joinMarketings) {
				...
				// Mandatory
				chcsTcVo.setChCd(OmniConstants.OSULLOC_OFFLINE_CHCD);
				chcsTcVo.setFscrId("OCP");
				chcsTcVo.setLschId("OCP");
				// Optional & Mandatory
				// 고객가입시 사용한 매체 관리를 위한 코드(W:WEB, M:MOBILE, A:APP)가 Web 일 경우 chcsNo 가 웹 아이디가 됨.
	...
	public static CreateCustChannelRequest buildIntegratedOfflineChannelCustomerData(final OnOffline onoffline, final Channel channel, final JoinRequest joinRequest) {
		...
		if (joinBpTerms != null && !joinBpTerms.isEmpty()) {
			for (Terms joinTerm : joinBpTerms) {
				com.amorepacific.oneap.auth.api.vo.ivo.CreateCustChannelRequest.CicuedCuTncaTcVo terms = new com.amorepacific.oneap.auth.api.vo.ivo.CreateCustChannelRequest.CicuedCuTncaTcVo();
				terms.setTcatCd(joinTerm.getTcatCd());
				terms.setTncvNo(joinTerm.getTncvNo());
				terms.setFscrId("OCP");
				terms.setLschId("OCP");
		...
		// 제휴사 멤버십 연동 시 추가 파라미터
		if (StringUtils.hasText(joinRequest.getPrcnLnkgStorNo())) {
			cicuedCuChTcVo.setPrcnLnkgStorNo(joinRequest.getPrcnLnkgStorNo());
		}
	 ...
	 * 
	 * <pre>
	 * comment  : 네이버스마트스토어 고객통합 경로 가입
	 * 
	 * author   : hjw0228
	 * date     : 2023. 05. 31. 오후 11:44:55
	 * </pre>
	 * @param onoffline
	 * @param channel
	 * @param joinRequest
	 * @return
	 */
	public static CreateCustChannelRequest buildIntegratedNaverCustomerData(final OnOffline onoffline, final Channel channel, final JoinRequest joinRequest) {
		...
		if (joinMarketings != null && !joinMarketings.isEmpty()) {
			for (Marketing joinMarketing : joinMarketings) {
				...
				if(joinRequest.getChcd().equals(joinMarketing.getChCd())) {
					// Mandatory
					// cicuemCuOptiTcVo.setChCd(joinMarketing.getChCd());
					cicuemCuOptiTcVo.setChCd(joinRequest.getChcd());
		...
		// 네이버 스마트 스토어 연동 시 401 채널이 아니고 401 채널에 미가입된 경우 401 채널도 강제 가입 처리
		if(!OmniConstants.NAVER_STORE_CHCD.equals(joinRequest.getChcd())) {
			cicuedCuChTcVo = new CicuedCuChTcVo();
			// Mandatory
			cicuedCuChTcVo.setIncsNo(joinRequest.getIncsno()); // 필수
			cicuedCuChTcVo.setChCd(OmniConstants.NAVER_STORE_CHCD); // 필수
			...
			if (joinMarketings != null && !joinMarketings.isEmpty()) {
				for (Marketing joinMarketing : joinMarketings) {
					if(OmniConstants.NAVER_STORE_CHCD.equals(joinMarketing.getChCd())) {
						
						// Mandatory
						cicuemCuOptiTcVo.setChCd(OmniConstants.NAVER_STORE_CHCD);
			...
			if (StringUtils.hasText(joinRequest.getPrcnLnkgStorNo())) { // 네이버 스마트 스토어(401) 인 경우 강제로 401 채널의 interlockSellerNo 주입 
				cicuedCuChTcVo.setPrcnLnkgStorNo(config.getChannelApi(OmniConstants.NAVER_STORE_CHCD, "sellerno", profile));
			} 
	...
	public static CreateCustChannelRequest buildIntegrated030ChannelCustomerData(final JoinRequest joinRequest) {
		...
		if (StringUtils.hasText(joinRequest.getLoginid())) { 
			...
			for (Object chCd : config.getBpChannelCodes()) { 
				...
				for(Marketing vo: joinRequest.getMarketings()) {
					if("030".equals(vo.getChCd()) && "Y".equals(vo.getSmsAgree())) {
						cicuemCuOptiTcVo.setSmsOptiYn("Y"); // 필수 SMS수신동의여부
						cicuemCuOptiTcVo.setSmsOptiDt(DateUtil.getCurrentDate()); // 필수 SMS수신동의일자 
					}
				}
