...
public class ApiOnlineProcessStep {
						...
						if (customers != null && customers.length > 0) {
							...
							if ("ICITSVCOM001".equals(customer.getRsltCd())) { // 없으면 등록 프로세스
								...
								if(certResult != null && "5".equals(joinRequest.getJoinType()) &&
									...
									response.setResultCode("ICITSVCOM001");
								} else {
	 ...
	 * comment  : 전환가입 프로세스
	 * author   : takkies
	 * date     : 2020. 9. 8. 오후 7:08:30
	 * </pre>
	 * 
	 * @param joinRequest
	 * @return
	 */
	public BaseResponse conversionCustomerProcess(final JoinRequest joinRequest) {
		final StopWatch stopwatch = new StopWatch("conversionCustomerProcess");
		...
		ConversionCustomerProcess crp = new ConversionCustomerProcess(object);

		BaseResponse success = crp.conversionCustomerStep(crp::updateIntegratedCustomerCallApi, null);

		if (ResultCode.SUCCESS.getCode().equals(success.getResultCode())) {
			success = crp.conversionCustomerStep(crp::registBeautyPointCustomerCallApi, crp::cancelIntegratedCustomerCallApi);
		}

		if (ResultCode.SUCCESS.getCode().equals(success.getResultCode())) {
			success = crp.conversionCustomerStep(crp::updateOmniCustomer, null);
		}
	...
	private class ConversionCustomerProcess {
		private Map<String, Object> request;

		private ConversionCustomerProcess(final Map<String, Object> request) {
			this.request = request;
		}

		public BaseResponse conversionCustomerStep(Function<Map<String, Object>, BaseResponse> func, Function<Map<String, Object>, BaseResponse> failed) {
			BaseResponse success = func.apply(request);
			if (!ResultCode.SUCCESS.getCode().equals(success.getResultCode()) && failed != null) {
				return failed.apply(request);
			}
		...
		public BaseResponse updateIntegratedCustomerCallApi(final Map<String, Object> request) {
			...
			log.debug(OmniUtil.getApiResultCode(response, "ConversionCustomerProcess.updateIntegratedCustomerCallApi"));
			if (stopwatch.isRunning()) {
				stopwatch.stop();
			}
			return response;
		...
		public BaseResponse cancelIntegratedCustomerCallApi(final Map<String, Object> request) {
			...
			log.debug(OmniUtil.getApiResultCode(response, "ConversionCustomerProcess.cancelIntegratedCustomerCallApi"));
			if (stopwatch.isRunning()) {
				stopwatch.stop();
			}
			return response;
		...
		public BaseResponse registBeautyPointCustomerCallApi(final Map<String, Object> request) {
			...
			try {
				...
				log.debug("▶▶▶▶▶▶ ② beautypoint trans customer : {} --> convs type : {}", joinRequest.isTransCustomer(), convsType);
			...
			log.debug(OmniUtil.getApiResultCode(response, "ConversionCustomerProcess.registBeautyPointCustomerCallApi"));
			if (stopwatch.isRunning()) {
				stopwatch.stop();
			}
			return response;
		...
		public BaseResponse updateOmniCustomer(final Map<String, Object> request) {
			...
			try {
				...
				ApiBaseResponse createResponse = null;

				if ("202".equals(convsType)) {
					// 202 --> id, pw, cpw -> 전환(아이디, 비밀번호 있음) TRNS_CH_MINE chcount > 0
					// 기존의 채널정보는 disable 처리
					joinUserData.setJoinFlag(OmniConstants.OMNI_JOIN_FLAG);
					createResponse = customerApiService.createUser(joinUserData); // 옴니회원등록
					...
					if (success) {
						...
						log.debug("▶▶▶▶▶▶ ⑤ [update omni user] transform user : {}", StringUtil.printJson(joinRequest));
						String incsNo = joinRequest.getIncsno();
				...
				} else if ("203".equals(convsType)) {
					// 203 --> id, pw, cpw -> 전환(아이디 변경 필요, 아이디 입력 받음) TRNS_CH_OTHER chcount > 0
					// 기존의 다른 사람 채널 정보는 disable 처리 하지 않음.
					joinUserData.setJoinFlag(OmniConstants.OMNI_JOIN_FLAG);
					createResponse = customerApiService.createUser(joinUserData); // 옴니회원등록
					log.debug("▶▶▶▶▶▶ [update omni user] create : {}", StringUtil.printJson(createResponse));
					success &= ResultCode.SUCCESS.getCode().equals(createResponse.getResultCode()) || ResultCode.USER_ALREADY_EXIST.getCode().equals(createResponse.getResultCode());
