	...
	$(document).ready(function() {
			...
			$('#agree-contents').load(OMNIEnv.ctx + '/search/agree-detail .agree-txt', data, function(response, status, xhr) {
				if (status === 'success') {
					...
					if (data.tcatCd.indexOf('01') > 0) {
						...
						poptitle = '개인정보 수집/이용 동의';
					} else if (data.tcatCd.indexOf('105') > 0 || data.tcatCd.indexOf('106') > 0 || data.tcatCd.indexOf('107') > 0 || data.tcatCd.indexOf('108') > 0) {
						poptitle = '개인정보 제3자 제공 동의(선택)';
					}
			 		OMNI.popup.open({
						id:'agress-popup',
					 	title:poptitle,
		 ...
		 * 뷰티포인트 약관 상세보기
		 */		
		$(document).on('click', '.btn_link_bp', function(e) {	
			$('#agree-contents').empty();
			var data = {
				type:$(this).data('type') + '',
				chCd:$(this).data('chcd') + ''
