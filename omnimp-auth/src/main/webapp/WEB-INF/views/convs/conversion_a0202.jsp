...
<!DOCTYPE html><!-- conversion a202 conversion new password -->
<html lang="ko">
<head>
        ...
        <form id='trnsForm' method='post' action=''>
          ...
          <div class="input_form">
            <span class="inp" id="password-span">
              <input type="password" id='loginpassword' class="inp_text" maxlength="16" placeholder="비밀번호 (영문 소문자, 숫자, 특수문자 조합 8-16자)"  title="비밀번호"/>
          ...
          <div class="input_form">
            <span class="inp" id="confirm-password-span">
              <input type="password" id='loginconfirmpassword' class="inp_text" maxlength="16" placeholder="비밀번호 확인"  title="비밀번호 확인"/>
              <button type="button" class="btn_del" ><span class="blind">삭제</span></button>
            </span>
          ...
          <div class="all_agree_box is_open">
            ...
            <c:if test="${not empty corptermslist}">
              ...
              <c:forEach items="${corptermslist}" var="corpterm" varStatus="corpstatus">
                <li>
                  <span class="checkboxA">
                    <input type="checkbox" id="i_agree_bp<c:out value="${corpterm.chCd}" /><c:out value="${corpstatus.count}" />" name="bpterms_check" <c:if test="${corpterm.tncAgrMandYn eq 'Y'}">class="required"</c:if> title='(<c:out value="${corpterm.tncAgrMandYnTxt}" />) '/>
