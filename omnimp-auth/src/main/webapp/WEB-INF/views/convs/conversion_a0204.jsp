    ...
    <section class="container">
      ...
      <div class="sec_join">
        ...
        <c:if test="${not empty bpuserlist}">
        	<c:forEach items="<c:out value='${bpuserlist}' />" var="bp" varStatus="index">
        <div class="user_info${fn:length(bpuserlist) > 1 ? ' bg_white' : ''}">
          <h3<c:if test="${fn:length(bpuserlist) == 1}"> class="fc_blue"</c:if>>
            ...
            <span class="radioA">
              <input type="radio" id="i_userid_<c:out value='${index.index}' />"<c:if test="${index.index == 0}"> checked</c:if> name="useridList" value="<c:out value='${bp.umUserName}' />"  title="통합 아이디"/>
              <label for="i_userid_<c:out value='${index.index}' />" ap-click-area="통합회원 전환 02 (통합,경로)" ap-click-name="통합회원 전환 02 (통합,경로) - 통합 아이디 목록 (<c:out value='${index.index}' />)" ap-click-data="통합 아이디"><em class="tit tit_w40">아이디</em> <c:out value='${bp.umUserName}' /></label>
            </span> 
          			</c:when>
          			<c:otherwise>
        ...
        <div class="user_info">
          ...
          <dl class="dt_w40">
        	...
        	<c:choose>
        		<c:when test="${fn:length(bpuserlist) > 1}">
		<p class="txt_l fc_blue">* 선택한 뷰티포인트 통합 아이디로 회원정보가 통합되고, 앞으로 뷰티포인트 통합 아이디와 비밀번호로만 로그인 가능합니다.</p>
        		</c:when>
        		<c:otherwise>
        <p class="txt_l fc_blue">* 뷰티포인트 통합 아이디로 회원정보가 통합되고, 앞으로 뷰티포인트 통합 아이디와 비밀번호로만 로그인 가능합니다.</p>
        		</c:otherwise>
        	</c:choose>
        <c:if test="${certType eq 'ipin'}">
        ...
        <form id='trnsForm' method='post' action=''>
          ...
          <div class="btn_submit">
            <button type="button" class="btnA btn_blue" id='do-convs' disabled ap-click-area="통합회원 전환 02 (통합,경로)" ap-click-name="통합회원 전환 02 (통합,경로) - 동의하고 통합회원 전환 버튼" ap-click-data="동의하고 통합회원 전환">동의하고 통합회원 전환</button>
