...
<!DOCTYPE html><!-- conversion terms a202 01 ch id special char login id(203) -->
<html lang="ko">
<head>
			...
			$.ajax({
				...
				success: function(data) {
					if (data.resultCode === '1') {
						...
						$('#trnsForm')
					  	.attr('action', OMNIEnv.ctx + '/convs/terms-finish-203-ch') // /convs/finish
					  	.submit();
  ...
  <div id="wrap" class="wrap">
    ...
    <section class="container">
      <div class="page_top_area">
        <h2>뷰티포인트 통합회원 전환을 위해 새로운 아이디와 비밀번호를 설정 해주세요.</h2>
      </div>
      <div class="sec_join">
        <div class="user_info">
        	...
        	<c:forEach items="${chterms}" var="chterm" varStatus="status">
                <li>
                 <span class="checkboxA">
                  ...
                  <a href="javascript:;" ap-click-area="통합회원 전환 01 (자체)" ap-click-name="통합회원 전환 01 (자체) - (<c:out value="${chterm.tncAgrMandYnTxt}"/>) '<c:out value="${chterm.tncTtl}"/>' 상세 보기 버튼" ap-click-data="(<c:out value="${chterm.tncAgrMandYnTxt}"/>) '<c:out value="${chterm.tncTtl}"/>' 상세 보기" class="btn_link" data-lnk="<c:out escapeXml="false" value="${chterm.tncTxtUrl}"/>" data-chcd="<c:out escapeXml="false" value="${chterm.chCd}"/>" data-lnkcd="<c:out escapeXml="false" value="${chterm.tcatCd}"/>" data-lnkno="<c:out escapeXml="false" value="${chterm.tncvNo}"/>"><span class="blind">자세히보기</span></a>
                </li>
