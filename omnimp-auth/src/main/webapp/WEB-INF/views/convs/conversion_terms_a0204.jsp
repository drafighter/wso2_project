...
<!DOCTYPE html><!-- conversion terms 204 -->
<html lang="ko">
<head>
  ...
  $(document).ready(function() {
	  ...
	  $("input[type=checkbox]:not(#all_chk)").change(function() {
		...
		$('#do-convs').on('click', function() { // A0205 통합회원전환완료
			...
			$('#trnsForm')
		  	.attr('action', OMNIEnv.ctx + '/convs/terms-finish-204')
		  	.submit();
      ...
      <div class="sec_join">
        ...
        <div class="user_info${fn:length(bpuserlist) > 1 ? ' bg_white' : ''}">
          <h3<c:if test="${fn:length(bpuserlist) == 1}"> class="fc_blue"</c:if>>
            ...
            <span class="radioA">
              <input type="radio" id="i_userid_${index.index}"<c:if test="${index.index == 0}"> checked</c:if> name="useridList" value="<c:out value='${bp.umUserName}' />"  title="'통합회원 아이디' 라디오 선택"/>
              <label for="i_userid_${index.index}" ap-click-area="통합회원 전환 02 (통합,경로)" ap-click-name="통합회원 전환 02 (통합,경로) - '통합회원 아이디' 라디오 선택" ap-click-data="'통합회원 아이디' 라디오 선택"><em class="tit tit_w40">아이디</em> <c:out value='${bp.umUserName}' /></label>
            </span>          			
          			</c:when>
          			<c:otherwise>
        ...
        <form id='trnsForm' method='post' action=''>
          ...
          <div class="all_agree_box is_open">
            ...
            <c:if test="${not empty corptermslist}">
              ...
              <c:forEach items="${corptermslist}" var="corpterm" varStatus="corpstatus">
                <li>
                  <span class="checkboxA">
                    <input type="checkbox" id="i_agree_<c:out value="${corpterm.chCd}"/><c:out value="${corpstatus.count}" />" name="bpterms_check" <c:if test="${corpterm.tncAgrMandYn eq 'Y'}">class="required"</c:if>title="(<c:out value="${corpterm.tncAgrMandYnTxt}" />) '<c:out value="${corpterm.tncTtl}" />'" />
                    <label for="i_agree_<c:out value="${corpterm.chCd}"/><c:out value="${corpstatus.count}" />" ap-click-area="통합회원 전환 02 (통합,경로)" ap-click-name="통합회원 전환 02 (통합,경로) - (<c:out value="${corpterm.tncAgrMandYnTxt}" />) '<c:out value="${corpterm.tncTtl}" />' 체크 박스" ap-click-data="(<c:out value="${corpterm.tncAgrMandYnTxt}" />) '<c:out value="${corpterm.tncTtl}" />'"><span class="checkbox_label">[<c:out value="${corpterm.tncAgrMandYnTxt}" />] <c:out value="${corpterm.tncTtl}" /></span></label>
                  </span>
          ...
          <div class="btn_submit">
            <button type="button" class="btnA btn_blue" p-click-area="통합회원 전환 02 (통합,경로)" ap-click-name="통합회원 전환 02 (통합,경로) - 동의하고 통합회원 전환 버튼" ap-click-data="동의하고 통합회원 전환" id='do-convs' disabled>동의하고 통합회원 전환</button>
