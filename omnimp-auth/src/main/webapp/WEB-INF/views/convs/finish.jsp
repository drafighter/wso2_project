...
<head>
  ...
  var goAction = function() {
	<c:choose>
		<c:when test="${convsType eq '204'}">
		location.href = OMNIEnv.ctx + '/go-login';			
		</c:when>
		<c:otherwise>
		$('#authForm')
		.attr('action', OMNIEnv.ctx + '/moveauth')
		.submit();			
		</c:otherwise>
	</c:choose>
  };
...
<body>
  ...
  <div id="wrap" class="wrap">
    ...
    <section class="container">
      ...
      <p class="txt_c">* 뷰티포인트 통합 아이디와 비밀번호로만 로그인 가능합니다.</p>
      <div class="btn_submit">
        <button type="button" class="btnA btn_blue" ap-click-area="통합회원 전환 완료 안내" ap-click-name="통합회원 전환 완료 안내 - 확인 버튼 (로그인 처리)" ap-click-data="확인" id='convs-finish'>확인</button>
      </div>			
			</c:otherwise>
		</c:choose>      
    </section>
