...
<head>
  ...
  $(document).ready(function() {
  	...
  	//선택약관 추가
  		var html = '<div id="marketingChcd_030" class="marketingChcd_030">';
		html += '<li>';
		html += '<span class="checkboxA select_agree_low">';
		if ($('#all_chk').is(':checked')) {
        html += "<input type='checkbox' id='i_agree_bp_marketing_2' name='marketing_check' checked title='(선택) 온라인 사이트 문자 수신 동의'/>";
		}else{
			if($('#i_agree_bp_top').is(':checked')){
				$("input[id='i_agree_chagree_mrk']").removeAttr('disabled');
				html += "<input type='checkbox' id='i_agree_bp_marketing_2' name='marketing_check' title='(선택) 온라인 사이트 문자 수신 동의'/>";
			}else{
				html += "<input type='checkbox' id='i_agree_bp_marketing_2' name='marketing_check' disabled='disabled' title='(선택) 온라인 사이트 문자 수신 동의'/>";
			}
			
		}
        html += '<label for="i_agree_bp_marketing_2" ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (선택) 온라인 사이트 문자 수신 동의 체크 박스" ap-click-data="(선택) 온라인 사이트 문자 수신 동의"><span class="checkbox_label">[선택] 온라인 사이트 문자 수신 동의</span></label>';
        html += '</span>';
        html += '</li>';
        html += "<input type='hidden' name='marketingChcd' value='030'/>";
        html += '</div>';
	    $('#marketingChcd_030_under').html(html).show();
	  ...
	  //관계사 개인정보 수집 및 이용동의 약관
	    ...
	    if ($('#all_chk').is(':checked')) {
	    	...
	    	$("input[id='i_agree_bp_top_channel']").prop('checked', false);
	    	if(!$('#i_agree_bp_top').is(':checked')){
	    	...
	    	if(!$('#i_agree_bp_top_channel').is(':checked')){
		    	$("input[id='i_agree_chagree_mrk_channel']").prop('checked', false);
				$("input[id='i_agree_chagree_mrk_channel']").removeAttr('checked');
				$("input[id='i_agree_chagree_mrk_channel']").attr('disabled', 'disabled');
	    	}
	    }
	...
	var clearTermContent = function() {
		...
		$('.marketingChcd_030').each(function() {
			$(this).remove();
		});
		$('.terms_marketing_li').each(function() {
			$(this).remove();
	    ...
	    html2 += "</span>";
	    html2 += '<a href="javascript:;" ap-click-area="약관동의" ap-click-name="약관동의 - (<c:out value="${terms_marketing.tncAgrMandYnTxt}" />) "<c:out value="${terms_marketing.tncTtl}" />" 상세보기 버튼" ap-click-data="(<c:out value="${terms_marketing.tncAgrMandYnTxt}" />) "<c:out value="${terms_marketing.tncTtl}" />" 상세보기" class="btn_link_bp" data-lnk="<c:out escapeXml="false" value="${terms_marketing.tncTxtUrl}"/>" data-chcd="<c:out escapeXml="false" value="${chCd}" />"  data-lnkcd="<c:out escapeXml="false" value="${terms_marketing.tcatCd}" />" data-type="N" data-lnkno="<c:out escapeXml="false" value="${terms_marketing.tncvNo}" />"><span class="blind">자세히보기</span></a>';
	...
	//뷰티포인트 광고성 정보 수신 동의
	  ...
	  //채널 정보 수신 동의
	  	var marketing_disabledOnOff_channel = function(allChecked) {
	  		var requiredCheckboxLength = $("input[id='i_agree_chagree_mrk_channel']").length;
			var checkedRequiredCheckboxLength = $("input[id='i_agree_chagree_mrk_channel']:checked").length;
			var condition = requiredCheckboxLength == checkedRequiredCheckboxLength;
			if (allChecked !== undefined) {
				condition = allChecked;
			}
			if (condition) {
				$("input[id='i_agree_chagree_mrk_channel']").removeAttr('disabled');
      ...
      <div class="sec_join">
		...
		<div class="integrateid">
          <span class="checkboxA">
            <input type="checkbox" id="integrateidcheck" name="integrateidcheck" ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - 통합 아이디 등록 (선택) 체크 박스" ap-click-data="통합 아이디 등록 (선택)" title="통합 아이디 등록 (선택)"/>
        ...
        <div id="area_int_id_reg" style="display: none;">
            ...
            <div class="agree_list">
              ...
              <ul>
                <li>
                  ...
                  <a href="javascript:;" class="btn_link_bp" data-type='S'  data-chcd="${chCd}"  ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (필수) '뷰티포인트 서비스 이용 약관' 상세 보기 버튼" ap-click-data="(필수) '뷰티포인트 서비스 이용 약관' 상세 보기 버튼">
                ...
                <li>
                  ...
                  <a href="javascript:;" class="btn_link_bp" data-type='C'  data-chcd="${chCd}"  ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (필수) '개인정보 이용 및 수집에 대한 동의' 상세 보기 버튼" ap-click-data="(필수) '개인정보 이용 및 수집에 대한 동의' 상세 보기 버튼">
                  <span class="blind">자세히보기</span></a>
                </li>
                ...
                <li>
                  <span class="checkboxA">
                    ...
                    <label for="i_agree_bpterms4" ap-click-area="정보입력 및 약관동의" data-chcd="${chCd}"  ap-click-name="정보입력 및 약관동의 - (선택) '개인정보 제 3자 제공 동의' 체크 박스" ap-click-data="(선택) '개인정보 제 3자 제공 동의'"><span class="checkbox_label">[선택] 개인정보 제 3자 제공 동의</span><em class="sm">*외부 컨텐츠 마케팅 활용</em></label>
                  </span>
                  <a href="javascript:;" class="btn_link_bp" data-type='A'  data-chcd="${chCd}"  ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (선택) '개인정보 제 3자 제공 동의' 상세 보기 버튼" ap-click-data="(선택) '개인정보 제 3자 제공 동의' 상세 보기 버튼">
                  <span class="blind">자세히보기</span></a>
                </li>
                ...
                <li>
                  ...
                  <a href="javascript:;" class="btn_link_bp" data-type='T'  data-chcd="${chCd}"  ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (선택) '국외 이전 동의' 상세 보기 버튼" ap-click-data="(선택) '국외 이전 동의' 상세 보기 버튼">
                  <span class="blind">자세히보기</span></a>
                </li>
              ...
              <ul id='bp-mrk'>
                ...
                <li>
	                <ul>
			            ...
			            <li id='marketingChcd_030_under' style='display:none'></li>
			            <c:if test="${empty terms_marketing }">
			            	<li>
					          <span class="checkboxA select_agree_low">
	                ...
	                <input type='hidden' name='bpTncvNos' value='${terms_marketing.tncvNo}'/>
	         		...
	         		<li>
		                <ul>
			                <li>
			                  <span class="checkboxA select_agree_low">
			                    <input type="checkbox" id="i_agree_chagree_mrk_channel" name='marketing_check' disabled='disabled' title="(선택) '문자 수신 동의'"/>
			                    <label for="i_agree_chagree_mrk_channel" ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (선택) '문자 수신 동의' 체크 박스" ap-click-data="(선택) '문자 수신 동의'"><span class="checkbox_label">[선택] <c:out value="${channelName}" />  문자 수신 동의</span></label>
			                  </span>
					        <input type='hidden' name='marketingChcd' value='<c:out value="${chcd}" />'/>
			                </li>
							<li>
          ...
          <div class="btn_submit">
            <button type="button" id="dojoin" class="btnA btn_blue" disabled=true  ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - 동의하고 가입 버튼" ap-click-data="동의하고 가입">동의하고 가입</button>
