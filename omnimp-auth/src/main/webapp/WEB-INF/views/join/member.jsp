	...
	<div id="wrap" class="wrap">
		<c:choose>
			<c:when test="${headertype eq 'cancelbtn'}">
				<common:header title="회원가입" gaArea="통합회원 가입 수단 선택" type="cancelbtn" />
			</c:when>
			<c:otherwise>
				<common:header title="회원가입" gaArea="통합회원 가입 수단 선택" type="prvclose" />
			</c:otherwise>
		</c:choose>
		...
		<section class="container">
			...
			<div class="join_main">
				<strong>회원가입 인증 방식을 선택해주세요.</strong>
				<div class="btn_joins">
				...
				<c:choose>
					<c:when test="${(offline and !isKakaoSyncEnable) or vtdisable or !isSsnsSignUpEnable or joinAditor}">
						<!-- <ul class="guide_list">
							<li>카카오 계정으로 간편 가입을 원하시면 매장에 있는 카카오 가입 QR 코드를 이용해주세요.</li>
						</ul> -->
						<c:if test="${wso2.channelCd eq '070' and !joinAditor}"><!-- 에딧샵: 에디터 가입 아닐 시 카카오 노출 -->
							<a href="javascript:;" class="btn_join_kakao"
							ap-click-area="통합회원 가입 수단 선택"
							ap-click-name="통합회원 가입 수단 선택 - 카카오 가입 버튼"
							ap-click-data="카카오 계정 간편 가입">카카오 계정 간편 가입</a>
						</c:if>
					</c:when>
					<c:otherwise>
						<a href="javascript:;" class="btn_join_kakao"
