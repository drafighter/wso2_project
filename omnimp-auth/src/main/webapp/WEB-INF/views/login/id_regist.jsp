...
<!DOCTYPE html><!-- ME-FO-A0207 id regist -->
<html lang="ko">
<head>
  ...
  $(document).ready(function() {
		...
		$('#do-regist-login').on('click', function() {
			...
			if (validId.code > 0 && validPw.code > 0) {
				...
				$('#joinform')
				.attr('action', OMNIEnv.ctx + '/join/setcomplete')
				.submit();
					...
					OMNI.popup.open(
							{
								...
								close:function() {
									...
									OMNI.popup.close({id:'regist-id-warn'});
									$.ajax({url:OMNIEnv.ctx + '/ga/tagging/join/stop/' + encodeURIComponent("경로가입"),type:'get'});
...
<body>
  ...
  <div id="wrap" class="wrap">
    ...
    <c:choose>
    	<c:when test="${joinStepType eq '35'}">
		<h2>뷰티포인트 통합 아이디를 등록해주세요.</h2>
		<c:if test="${sessionScope.isMembership ne 'Y'}">
        <p>통합 아이디를 등록하면 아모레퍼시픽 뷰티포인트와 <c:out value="${channelName}" /> 회원으로 가입되며, 온라인 서비스를 이용할 수 있습니다.</p>
        </c:if>
       </div>
       <div class="">
    	...
    	<c:otherwise>
        <h2>뷰티포인트 통합 아이디를 등록해주세요.</h2>
        <p>뷰티포인트 통합 아이디를 등록하면 아모레퍼시픽 브랜드의 온라인 서비스를 이용할 수 있습니다.</p>
        </div>    	
    	</c:otherwise>
    </c:choose>
      <form id='joinform' method='post' action=''>
			...
			<input type='hidden' name='joinStepType' value='<c:out value="${joinStepType}" />'/>
        ...
        <div class="btn_submit ver2">
          ...
          <c:choose>
			<c:when test="${joinAditor}">
				<button type="button" class="btnA btn_blue" id='do-regist-login' disabled ap-click-area="통합 아이디 등록" ap-click-name="통합 아이디 등록 - 등록하기 버튼" ap-click-data="등록하기">등록하기</button>
			</c:when>
			<c:otherwise>
