...
<head>
  ...
  var goAction = function() {
	  ...
	  } else if (joinType === '35') { // JOINED_OFF 35 : A0103 --> A0105 
		  $('#termsForm').attr('action', OMNIEnv.ctx + '/join/go-idregist').submit();
	  	  //location.href = OMNIEnv.ctx + '/join/step';
		  //$('#termsForm').attr('action', OMNIEnv.ctx + '/join/go-terms').submit();
	  } else {
  ...
  <div id="wrap" class="wrap">
    ...
    <section class="container">
      ...
      <div class="user_info mb13">      
      	...
      	<h3>
          <span class="radioA">
            <input type="radio" id="i_userid_${index.index}" name="i_userid_list" <c:if test="${index.index == 0}">checked</c:if> value="${user.chcsNo}" title="로그인 아이디 선택"/>
            <label for="i_userid_${index.index}" ap-click-area="가입된 회원" ap-click-name="가입된 회원 - 로그인 아이디 선택 라디오 버튼" ap-click-data="로그인 아이디 선택"><em class="tit tit_w33">아이디</em><c:out value="${user.chcsNo}"/></label>
          </span>      	
      	</h3>
      	</c:if>
        <dl class="dt_w33">
