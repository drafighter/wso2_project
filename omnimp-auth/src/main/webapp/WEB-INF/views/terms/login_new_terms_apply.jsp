  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#term-agree-ok').on('click', function() {
			...
			$('#termsForm')
			.attr('action', OMNIEnv.ctx + '/join/terms/applycorp') // /join/finish/applycorp
			.submit();
		});
	});
	  ...
	  var closeAction = function() {
			...
			OMNI.popup.open({
				...
				close: function() {
					...
					OMNI.popup.close({id:'next-warn'});
					$.ajax({url:OMNIEnv.ctx + '/ga/tagging/join/stop/' + encodeURIComponent("약관동의(경로)"),type:'get'});
            ...
            <div class="agree_list bdt_n">
              ...
              <ul>
                ...
                <li>
                  ...
                  <a href="javascript:;" class="btn_link_bp" data-type='P' ap-click-area="약관동의" ap-click-name="약관동의 - (<c:out value="${term.tncAgrMandYnTxt}" />) '<c:out value="${term.tncTtl}" />' 상세보기 버튼" ap-click-data="(<c:out value="${term.tncAgrMandYnTxt}" />) '<c:out value="${term.tncTtl}" />' 상세보기" data-lnk="<c:out escapeXml="false" value="${term.tncTxtUrl}"/>" data-chcd="<c:out escapeXml="false" value="${term.chCd}" />" data-lnkcd="<c:out escapeXml="false" value="${term.tcatCd}" />" data-lnkno="<c:out escapeXml="false" value="${term.tncvNo}" />"><span class="blind">자세히보기</span></a>
                </li>
