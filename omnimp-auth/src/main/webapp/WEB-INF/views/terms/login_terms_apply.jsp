...
<!DOCTYPE html><!-- login terms apply -->
<html lang="ko">
<head>
  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#term-agree-ok').on('click', function() {
			...
			$('#termsForm')
			.attr('action', OMNIEnv.ctx + '/join/terms/apply') // /join/finish/apply
			.submit();
		});
          ...
          <div class="all_agree_box m0 is_open">
        	...
        	<c:forEach items="${terms}" var="term" varStatus="status">
                <li>
                  ...
                  <a href="javascript:;" ap-click-area="약관동의" ap-click-name="약관동의 - (<c:out value="${term.tncAgrMandYnTxt}" />) '<c:out value="${term.tncTtl}" />' 상세보기 버튼" ap-click-data="(<c:out value="${term.tncAgrMandYnTxt}" />) '<c:out value="${term.tncTtl}" />' 상세보기" class="btn_link" data-lnk="<c:out escapeXml="false" value="${term.tncTxtUrl}"/>" data-chcd="<c:out escapeXml="false" value="${term.chCd}" />" data-lnkcd="<c:out escapeXml="false" value="${term.tcatCd}" />" data-lnkno="<c:out value="${term.tncvNo}" />"><span class="blind">자세히보기</span></a>
                </li>
              ...
              <ul>
                <li>
                  <span class="checkboxA">
                    ...
                    <label for="i_agree_bp_top_channel" ap-click-area="정보입력 및 약관동의" ap-click-name="정보입력 및 약관동의 - (선택) '개인정보 수집 및 이용동의 (마케팅)'" ap-click-data="(선택) '개인정보 수집 및 이용동의 (마케팅)'"><span class="checkbox_label">[선택] 개인정보 수집 및 이용동의 (마케팅) (<c:out value="${channelName}" />)</span><em class="sm">*개인정보 수집 및 이용(마케팅)에 동의 하셔야 문자 수신 동의가 가능합니다.</em></label>
                  </span>
              ...
              <c:if test="${empty terms && isMarketing}">
              <strong class="txt_t"><c:out value="${channelName}" /> 광고성 정보 수신 동의</strong>
              <ul>
                <li>
                  <span class="checkboxA">
                    <input type="checkbox" id="i_agree_chagree2" name="marketing_check" title="(선택) '문자 수신동의'"/>
                    <label for="i_agree_chagree2" ap-click-area="약관동의" ap-click-name="약관동의 - (선택) '문자 수신동의' 체크 박스" ap-click-data="(선택) '문자 수신동의'"><span class="checkbox_label">[선택] 문자 수신동의</span></label>
                  </span>
                <input type='hidden' name='marketingChcd' value='<c:out value="${chcd}" />'/>
                </li>
				<li>
