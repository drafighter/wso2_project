  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#term-agree-ok').on('click', function() {
			...
			//ga tagging cookie set 20211231
		//	$.ajax({url:OMNIEnv.ctx + '/ga/tagging/cookie',type:'get'});	
			...
			$('#termsForm')
			.attr('action', OMNIEnv.ctx + '/join/finish/apply_ch')
			.submit();
		});
          ...
          <div class="all_agree_box m0 is_open">
        	...
        	<c:forEach items="${terms}" var="term" varStatus="status">
                <li>
                  ...
                  <a href="javascript:;" ap-click-area="약관동의" ap-click-name="약관동의 - (<c:out value="${term.tncAgrMandYnTxt}"/>) '<c:out value="${term.tncTtl}"/>' 상세보기 버튼" ap-click-data="(<c:out value="${term.tncAgrMandYnTxt}"/>) '<c:out value="${term.tncTtl}"/>' 상세보기" class="btn_link" data-lnk="<c:out value="${term.tncTxtUrl}"/>" data-chcd="<c:out value="${term.chCd}"/>" data-lnkcd="<c:out value="${term.tcatCd}"/>" data-lnkno="<c:out value="${term.tncvNo}"/>"><span class="blind">자세히보기</span></a>
                </li>
